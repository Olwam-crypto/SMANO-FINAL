...
# 1. Capture the address from the JavaScript evaluation
user_address = streamlit_js_eval(js_expressions="window.ethereum.request({ method: 'eth_requestAccounts' }).then(accounts => accounts[0])", key="connect_wallet")

# 2. Update the sidebar based on whether an address was found
if user_address:
    st.sidebar.success(f"Connected: {user_address[:6]}...{user_address[-4:]}")
    # Optional: Save to session state so other pages can see it
    st.session_state['user_address'] = user_address
else:
    st.sidebar.warning("MetaMask not connected. Please connect to interact with the platform.")
    # No need for an 'if st.sidebar.button' here because streamlit_js_eval 
    # will trigger the popup automatically or you can keep the button to wrap it.
# ==========================================
...
if selection == "🏠Overview / Dashboard":
    ...
    with st.spinner("Fetching live network data..."):
        try:
            # Call read functions from the blockchain // edited
            admin_address = to_checksum_address(contract.functions.admin().call())
            ...
            admin_address = Web3.to_checksum_address(admin_address) #edited
...
# ==========================================
    ...
    if not is_registered:
        ...
        if st.button("Register as Donor"):
            user_address = Web3.to_checksum_address(user_address) #edited
            if user_address:
                # Build the transaction payload for MetaMask
                tx_data = contract.functions.registerDonor().build_transaction({
    'from': user_address,
    'nonce': w3.eth.get_transaction_count(user_address), #edited
    # Add other tx parameters like gas if needed
})['data']
...
# ==========================================
    ...
    "applyAsSupplier",
    args=[contact_name, company_name, country, region]
) #edited
                    trigger_metamask_tx(tx_data)
